"""WorldQuant Brain 批量Alpha生成系统"""

from brain_batch_alpha import BrainBatchAlpha
from dataset_config import get_dataset_list, get_dataset_by_index
import os

def submit_alpha_ids(brain, num_to_submit=2):
    """提交保存的Alpha ID"""
    try:
        if not os.path.exists('alpha_ids.txt'):
            print("❌ 没有找到保存的Alpha ID文件")
            return
            
        with open('alpha_ids.txt', 'r') as f:
        ...
        with open('alpha_ids.txt', 'w') as f:
            f.writelines([f"{id}\n" for id in remaining_ids])
            
    except Exception as e:
        print(f"❌ 提交Alpha时出错: {str(e)}")

def main():
    """主程序入口"""
    try:
        print("🚀 启动 WorldQuant Brain 批量Alpha生成系统")
        
        print("\n📋 请选择运行模式:")
        print("1: 自动模式 (测试并自动提交2个合格Alpha)")
        print("2: 仅测试模式 (测试并保存合格Alpha ID)")
        print("3: 仅提交模式 (提交已保存的合格Alpha ID)")
